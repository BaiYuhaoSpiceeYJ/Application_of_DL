...
"""
Deep Q Learning 例子的 Maze（迷宫） 环境
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.n_features = 2  # state/observation 里的特征数目
        self.title('Deep Q Learning')
    ...
    def step(self, action):
        ...
        self.canvas.move(self.robot, base_action[0], base_action[1])  # 移动机器人

        state_ = self.canvas.coords(self.robot)  # 下一个 state

        # 奖励机制
        if state_ == self.canvas.coords(self.treasure):
            reward = 1  # 找到宝藏，奖励为 1
            done = True
            print("找到宝藏，好棒!")
        elif state_ == self.canvas.coords(self.bomb1):
            reward = -1  # 踩到炸弹1，奖励为 -1
            done = True
            print("炸弹 1 爆炸...")
        elif state_ == self.canvas.coords(self.bomb2):
        ...
        s_ = (np.array(state_[:2]) - np.array(self.canvas.coords(self.treasure)[:2])) / (HEIGHT * UNIT)
